...
namespace Shpellbook
{
    public class Eval
    {
        ...
        {
            if (builtins.ContainsKey(command.args[0])) return EvaluateBuiltin(command).Result;
            return EvaluatePath(command).Result;
        }
        ...
        public static Task<int> EvaluateBuiltin(Command command)
        {
            ...
            {
                Func<string[], int> res;
                builtins.TryGetValue(command.args[0], out res);
